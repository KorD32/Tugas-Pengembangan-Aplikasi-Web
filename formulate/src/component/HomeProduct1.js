...
import ProductCosmetic from "../data/ProductCosemetic";
...
export const HomeProduct1 = () => {
  ...
  const nextProduct = () => {
    setActiveIndex((prevIndex) =>
      prevIndex < ProductCosmetic.length - productsPerPage
        ? prevIndex + 1
        : 0
    );
  };
  ...
  const prevProduct = () => {
    setActiveIndex((prevIndex) =>
      prevIndex > 0
        ? prevIndex - 1
        : ProductCosmetic.length - productsPerPage
    );
  };

  const displayedProducts = ProductCosmetic.slice(
    activeIndex,
